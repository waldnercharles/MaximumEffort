 ...
 */
	...
	using MultipartFormDataProviderItems =
		std::vector<MultipartFormDataProvider>;
	...
	public:
		...
		using MultipartReader = std::function<
			bool(MultipartContentHeader header, ContentReceiver receiver)>;
		...
		bool operator()(MultipartContentHeader header, ContentReceiver receiver)
			const
		{
	...
	struct Request
	{
		...
		std::string
		get_header_value(const std::string &key, size_t id = 0) const;
		uint64_t
		get_header_value_u64(const std::string &key, size_t id = 0) const;
		...
		std::vector<MultipartFormData> get_file_values(const std::string &key
		) const;
 ...
 */
	...
	public:
		...
		using ExceptionHandler = std::function<
			void(const Request &, Response &, std::exception_ptr ep)>;
	...
	private:
		...
		using HandlersForContentReader = std::vector<std::pair<
		...
		bool write_response_with_content(
			Stream &strm,
			bool close_connection,
			const Request &req,
			Response &res
	...
	public:
		...
		std::string
		get_request_header_value(const std::string &key, size_t id = 0) const;
		uint64_t get_request_header_value_u64(
	...
	public:
		...
		void set_proxy_basic_auth(
			const std::string &username,
			const std::string &password
		);
		void set_proxy_bearer_token_auth(const std::string &token);
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
		void set_proxy_digest_auth(
			const std::string &username,
			const std::string &password
		);
#endif
 ...
 */
	...
	namespace detail
	{
		...
		inline uint64_t get_header_value_u64(
			const Headers &headers,
			const std::string &key,
			size_t id,
			uint64_t def
	...
	inline uint64_t
	Request::get_header_value_u64(const std::string &key, size_t id) const
	{
		return detail::get_header_value_u64(headers, key, id, 0);
	}

	inline uint64_t
	Response::get_header_value_u64(const std::string &key, size_t id) const
	{
		return detail::get_header_value_u64(headers, key, id, 0);
	}
	...
	inline uint64_t Result::get_request_header_value_u64(
		const std::string &key,
		size_t id
	) const
	{
 ...
 */
	...
	namespace detail
	{
		...
		public:
			...
			bool
			decompress(const char *data, size_t data_length, Callback callback)
				override;
 ...
 */
	...
	namespace detail
	{
				...
				switch (s[i])
				{
					...
					default:
						...
						if (c >= 0x80)
						{
							...
							char hex[4];
							auto len =
								snprintf(hex, sizeof(hex) - 1, "%02X", c);
							assert(len == 2);
		...
		inline stream_line_reader::stream_line_reader(
			Stream &strm,
			char *fixed_buffer,
			size_t fixed_buffer_size
		)
		...
		inline bool mmap::open(const char *path)
		{
			...
			hMapping_ =
				::CreateFileMapping(hFile_, NULL, PAGE_READONLY, 0, 0, NULL);
...
#define USE_IF2IP
		...
		get_remote_ip_and_port(socket_t sock, std::string &ip, int &port)
		{
				...
				))
			{
				...
				if (addr.ss_family == AF_UNIX)
				{
					...
					if (getsockopt(sock, SOL_LOCAL, SO_PEERPID, &pid, &len) ==
						0)
					{
						port = pid;
					}
		...
		inline bool nocompressor::compress(
			const char *data,
			size_t data_length,
			bool /*last*/,
			Callback callback
		...
		inline bool gzip_compressor::compress(
			const char *data,
			size_t data_length,
			bool last,
			Callback callback
		...
		inline bool gzip_decompressor::decompress(
			const char *data,
			size_t data_length,
			Callback callback
		)
		...
		inline bool brotli_compressor::compress(
			const char *data,
			size_t data_length,
			bool last,
			Callback callback
		...
		inline bool brotli_decompressor::decompress(
			const char *data,
			size_t data_length,
			Callback callback
		)
		...
		inline bool
		compare_case_ignore(const std::string &a, const std::string &b)
		{
			if (a.size() != b.size())
			{
				return false;
		...
		)
		{
			...
			while (r < len)
			{
				...
				auto n = strm.read(
					buf,
					(std::min)(read_len, CPPHTTPLIB_RECV_BUFSIZ)
				);
				if (n <= 0)
				{
					return false;
		...
		inline bool read_content_without_length(
			Stream &strm,
			ContentReceiverWithProgress out
		)
		{
		...
		template <typename T>
		inline bool read_content_chunked(
			Stream &strm,
			T &x,
			ContentReceiverWithProgress out
		)
		...
		inline bool parse_multipart_boundary(
			const std::string &content_type,
			std::string &boundary
		)
		{
			...
			boundary =
				trim_double_quotes_copy(content_type.substr(beg, end - beg));
				...
				while (buf_size() > 0)
				{
					switch (state_)
					{
						...
						{ // Body
							...
							{
								auto len =
									buf_size() - crlf_dash_boundary_.size();
								if (len > 0)
								{
		...
		)
		{
			for (size_t i = 0; i < req.ranges.size(); i++)
			{
				...
				stoken(
					make_content_range_header_field(range, res.content_length_)
				);
				ctoken("\r\n");
				ctoken("\r\n");

				auto offsets =
					get_range_offset_and_length(req, res.content_length_, i);
...
#if TARGET_OS_OSX
		template <typename T>
		using CFObjectPtr = std::unique_ptr<
		...
		inline bool add_certs_to_x509_store(CFArrayRef certs, X509_STORE *store)
		{
			...
			for (auto i = 0; i < CFArrayGetCount(certs); ++i)
			{
				...
				CFObjectPtr<CFDataRef> cert_data_ptr(
					cert_data,
					cf_object_ptr_deleter
				);
		...
		inline bool parse_www_authenticate(
			const Response &res,
	...
	inline std::string
	Request::get_header_value(const std::string &key, size_t id) const
	{
	...
	inline MultipartFormData Request::get_file_value(const std::string &key
	) const
	{
	...
	inline std::string
	Response::get_header_value(const std::string &key, size_t id) const
	{
		return detail::get_header_value(headers, key, id, "");
	}
	...
	inline void Response::set_content(
		const char *s,
		size_t n,
		const std::string &content_type
	)
	...
	inline size_t Result::get_request_header_value_count(const std::string &key
	) const
	{
	...
	namespace detail
	{
		...
		inline void BufferStream::
			get_remote_ip_and_port(std::string & /*ip*/, int & /*port*/) const
		{
		}

		inline void BufferStream::
			get_local_ip_and_port(std::string & /*ip*/, int & /*port*/) const
		{
		}
		...
		inline PathParamsMatcher::PathParamsMatcher(const std::string &pattern)
		{
			...
			while (true)
			{
				...
				auto param_name = pattern.substr(
					param_name_start,
		...
		inline bool PathParamsMatcher::match(Request &request) const
		{
			...
			for (size_t i = 0; i < static_fragments_.size(); ++i)
			{
				...
				// comparison
				if (std::strncmp(
						request.path.c_str() + starting_pos,
						fragment.c_str(),
						fragment.length()
					) != 0)
				{
	...
	{
		post_handlers_for_content_reader_.emplace_back(
	...
	{
		put_handlers_for_content_reader_.emplace_back(
			make_matcher(pattern),
			std::move(handler)
		);
		return *this;
	...
	{
		patch_handlers_for_content_reader_.emplace_back(
			make_matcher(pattern),
			std::move(handler)
		);
		return *this;
	...
	{
		delete_handlers_for_content_reader_.emplace_back(
			make_matcher(pattern),
			std::move(handler)
		);
		return *this;
	...
	{
		options_handlers_.emplace_back(
			make_matcher(pattern),
			std::move(handler)
		);
		return *this;
	...
	inline bool Server::set_mount_point(
		const std::string &mount_point,
		const std::string &dir,
		Headers headers
	)
			...
			detail::split(
				...
				[&](const char *b, const char *e) {
					switch (count)
					{
						case 0:
							req.path =
								detail::decode_url(std::string(b, e), false);
							break;
						case 1:
						{
							if (e - b > 0)
	...
	inline bool Server::write_response(
		Stream &strm,
		bool close_connection,
		const Request &req,
		Response &res
	...
	) const
	{
      ...
      return true;
      */
				return multipart_form_data_parser
					.parse(buf, n, multipart_receiver, multipart_header);
			};
		}
		else
		{
	...
	Server::handle_file_request(const Request &req, Response &res, bool head)
	{
		for (const auto &entry : base_dirs_)
		{
			// Prefix match
			if (!req.path
					 .compare(0, entry.mount_point.size(), entry.mount_point))
			{
				std::string sub_path =
				...
				if (detail::is_valid_path(sub_path))
				{
					...
					if (detail::is_file(path))
					{
						...
						res.set_content_provider(
							...
							[mm](size_t offset, size_t length, DataSink &sink)
								-> bool {
	...
	inline bool Server::dispatch_request(
		Request &req,
		Response &res,
	...
	inline void Server::apply_ranges(
		const Request &req,
		Response &res,
		std::string &content_type,
		std::string &boundary
	...
	inline bool ClientImpl::read_response_line(
		Stream &strm,
		const Request &req,
		Response &res
	) const
	...
	inline bool ClientImpl::write_content_with_provider(
		Stream &strm,
		const Request &req,
		Error &error
	) const
	...
	ClientImpl::get_multipart_content_provider(
		const std::string &boundary,
		const MultipartFormDataItems &items,
		const MultipartFormDataProviderItems &provider_items
	) const
	{
		...
		return [&,
				cur_item,
				cur_start](size_t offset, DataSink &sink) mutable -> bool {
			...
			else if (cur_item < provider_items.size())
			{
				...
				if (!provider_items[cur_item]
						 .provider(offset - cur_start, cur_sink))
				{
					return false;
				}
	...
	inline void ClientImpl::set_basic_auth(
		const std::string &username,
		const std::string &password
	)
	{
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
	inline void ClientImpl::set_digest_auth(
		const std::string &username,
		const std::string &password
	)
	{
	...
	inline void ClientImpl::set_proxy_basic_auth(
		const std::string &username,
		const std::string &password
	)
	{
	...
	inline void ClientImpl::set_ca_cert_path(
		const std::string &ca_cert_file_path,
	...
	inline X509_STORE *ClientImpl::create_ca_cert_store(
		const char *ca_cert,
		std::size_t size
	) const
	{
 ...
 */
	...
	namespace detail
	{
		...
		inline void SSLSocketStream::get_remote_ip_and_port(
			std::string &ip,
			int &port
		) const
		{
  ...
  */
	...
	inline bool SSLClient::verify_host_with_subject_alt_name(X509 *server_cert
	) const
	{
		auto ret = false;
	...
	{
		return cli_
			->Get(path, std::move(content_receiver), std::move(progress));
	}
	inline Result Client::Get(
		const std::string &path,
		const Headers &headers,
	...
	inline void Client::set_basic_auth(
		const std::string &username,
		const std::string &password
	)
	{
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
	inline void Client::set_digest_auth(
		const std::string &username,
		const std::string &password
	)
	{
	...
	inline void Client::set_proxy_basic_auth(
		const std::string &username,
		const std::string &password
	)
	{
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
	inline void Client::set_proxy_digest_auth(
		const std::string &username,
		const std::string &password
	)
	{
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
	inline void Client::set_ca_cert_path(
		const std::string &ca_cert_file_path,
		const std::string &ca_cert_dir_path
	)
	{
